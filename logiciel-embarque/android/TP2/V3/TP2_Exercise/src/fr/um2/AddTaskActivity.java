...
public class AddTaskActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		categories.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View view,
				...
				String[] projection = { CategoryEntry.FULL_ID, CategoryEntry.COLUMN_NAME_CATEGORY_NAME};
				
				// Define 'where' part of query.
				String selection = CategoryEntry.COLUMN_NAME_CATEGORY_NAME + " = '" + category + "'";

				Cursor cursor = db.query(CategoryEntry.TABLE_NAME, projection,
						selection, null, null, null, null);
				
				if(cursor != null){
					while(cursor.moveToNext()){
						categoryId = cursor.getInt(0);
						break; // get first find
					}
				}
	...
	public void addTask(View view) {
		taskName = ((EditText) findViewById(R.id.edit_task_name)).getText()
				.toString();
		Log.println(Log.INFO, "Task", taskName + " , " + date + " , "
		...
		values.put(TaskEntry.COLUMN_NAME_CATEGORY_ID, categoryId);

		// Insert the new row, returning the primary key value of the new row
		long newTaskId = db.insert(TaskEntry.TABLE_NAME, null, values);
		
		Toast.makeText(this, getResources().getString(R.string.add_task) + " " + taskName, Toast.LENGTH_SHORT).show();
